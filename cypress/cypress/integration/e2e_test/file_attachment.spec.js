...
it('Files Upload1 - attach images', () => {
  ...
  cy.wait(1000)
  cy.xpath("//input[contains(@class,'rfu-file-input')]")
    .attachFile([imagefile,imagefile1,imagefile2])
...
it('Files Upload3 - attach video file', () => { // TO DO
  const special = 'Cypress End-to-End Testing.mp4'
  cy.fixture(special, 'binary')
    .then(Cypress.Blob.binaryStringToBlob)
    .then(fileContent => {
      cy.xpath("//input[contains(@class,'rfu-file-input')]").attachFile({fileContent,filePath: special,encoding: 'utf-8',lastModified: new Date().getTime()})
      cy.xpath("//textarea[contains(@placeholder,'Type your message')]").focus()
...
it('Files Upload4 - attach attach multiples', () => {
  ...
  cy.waitFor(1000)
  cy.xpath("//input[contains(@class,'rfu-file-input')]")
    .attachFile([imagefile1,jsonfile,textfile])
  cy.xpath("//textarea[contains(@placeholder,'Type your message')]").focus()
