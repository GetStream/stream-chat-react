...
describe('ChannelListTeam', () => {
  ...
  it('should render <ChatDown /> if error prop is true', () => {
    const { getByTestId } = render(
      <ChannelListTeam client={chatClient} error />,
    );

    const chatDown = getByTestId('chat-down');
    expect(chatDown).toBeTruthy();
  });

  it('should render <LoadingChannels /> if loading prop is true', () => {
    const { getByTestId } = render(
      <ChannelListTeam client={chatClient} loading />,
    );

    const loadingChannels = getByTestId('loading-channels');
    expect(loadingChannels).toBeTruthy();
  });
