...
[`ThreadProvider`](../components/contexts/thread-context.mdx#threadprovider) is an "adapter" which allows existing [`Thread`](../components/core-components/thread.mdx) component to consume methods and state of the [`Thread`](https://github.com/GetStream/stream-chat-js/blob/master/src/thread.ts) instance.
...
We know that component overrides are 90% of our SDK and providing these overrides only through [`Channel`](../components/core-components/channel.mdx) component isn't optimal. In the upcoming minor releases we'll be deprecating component overrides through individual component props and rather provide these overrides through `WithComponents`, you can read more about this component in our [`WithComponents` documentation](../components/contexts/component-context.mdx#with-components).

## Dropped ComponentContext Defaults

These components are no longer provided through the [`ComponentContext`](../components/contexts/component-context.mdx) as default values but have moved to their respective components:
