...
sidebar_position: 6
title: MessageList Notifications
---

import CustomNotification from '../assets/CustomNotification.png';

In this example, we demonstrate how to use the method `addNotification` pulled from the [`ChannelActionContext`](../contexts/channel_action_context#addNotification) in order to add a custom notification message to the `MessageList` at our desired time specification.

## addNotification Method

This method receives two arguments: text and type. The text parameter is the notification text, and the type argument is of type string and is either 'success' or 'error'. The type determines the style added.
...
:::note
Since `addNotification` is drawn from the `ChannelActionContext`, we must create an inner component that is a child of `Channel` and
