...
Inside of the `store` object replace the empty `createCall` implementation with this:
...
The `joinCall` function is pretty similar to the `createCall` function only that we’re now taking the `callId` we save in the channel `data` object and using it to call the `getCallToken` function on the `client` object. Then we join the call as before. Here is the code to replace the current (empty) `joinCall` function with:
...
With that we have all the logic available, there is just one more thing we’ll add. In order for us to not be stuck in a call when the window is closed, we’ll also leave the call when the `window.onunload` event is happening:
...
Next, we’ll use the `VideoContext` to import the necessary functions we need and create three helper functions:

1. `onVideoButtonClick`: When the user clicks on the video button there should be different actions called depending on the current state of video from the context
2. `callArea`: to show the correct button(s) or state indicators depending on the current `connectionState`
3. `buttonText`: depending on the state of connection we will re-use the button to show different text
