...
The event type the `ChannelList` reacts to and its corresponding behavior can be overridden using the appropriate prop. Let's take an example of ChannelList component for [frozen channels](https://getstream.io/chat/docs/javascript/freezing_channels/).
