...
Here is a guide to build these two UI components to use to override the defaults.
...
To use a custom component (or a different library component) for either the `LoadingErrorIndicactor` or `LoadingIndicator` props here (and in the default list component, too),
...
adding addtional UI items, not using all of the props, and incorporating other logic that fit your requirements. Check out the [CSS section](../customization/theming-and-css.mdx) for a note on custom styling.
...
Ultimately, these UI components are rendered via the [`ChannelPreview`](../utility-components/channel-preview.mdx).
wrapper component.
...
Let's take advantage of the SDK's [CSS classes](../customization/theming-and-css.mdx) for styling. We'll import the bundled CSS into the file where the application is instantiated then apply a class to the button element.
...
export const CustomPreview = (props: ChannelPreviewUIComponentProps) => {
    ...
    return (
        <div
        className={
            selected ? 'channel-preview__container selected' : 'channel-preview__container'
        }
