...
Live stream chat UI poses several hard technological challenges since users can spend a few hours watching a certain event. For that period, the chat UI should be able to handle a constant stream of messages, reactions, and media attachments. 
...
In this article, we will go through a best practices and tweaks for configuring the virtualized message list. 
...
This could be a potential pitfall if you're using a custom message component. The virtualized message list uses `getBoundingClientRect` ([MDN](https://developer.mozilla.org/en-US/docs/Web/API/Element/getBoundingClientRect)) on the outermost message element. This method does not measure CSS margins, which are tricky to measure in the first place [due to collapsing](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Box_Model/Mastering_margin_collapsing)). If the custom message has vertical margins that affect its height, the message list might behave erratically (blinking, rendering white areas, etc). 
...
If you're using a custom message, make sure that adding reactions to the message does not increase the message height. Good examples for such design would be the chat boxes of Twitch and Telegram. 
