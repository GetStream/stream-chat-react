...
:::important
When using a custom channel type these permissions must be set for each user type.
:::

If you want to use a different user permissions or create a custom channel type, the new permissions would need to be passed to `MessageList` using the `pinPermissions` prop.
...
To utilize pinned messages we will create a basic message component as well as a custom pin indicator to display when a message has been pinned.

Using the same message prop as the default [`PinIndicator`](https://github.com/GetStream/stream-chat-react/blob/master/src/components/Message/icons.tsx) we can easily pull the pinned state off of the message object. We will then create a very basic component that simple displays the text 'pinned'. This is where you could get very creative with custom icons and styles.

```tsx
export const CustomPinIndicator = (props) => {
  const { message } = props;
...
This component can now be passed to `Channel` and later pulled from the component context when used.

```tsx
<Channel PinIndicator={CustomPinIndicator}>{/* children of Channel component */}</Channel>
```

Now we will build a very basic message component that changes its background from turquoise to yellow when a message is pinned. Our custom pin indicator will also replace the `MessageOptions` unless the message is currently being hovered.
...
<img src={CustomPinMessage} alt='Custom Pin Indicator UI Component for Chat' width='700' />

Pinned Message (not hovered):

<img src={CustomPinMessagePinned} alt='Custom Pin Indicator UI Component for Chat' width='700' />
