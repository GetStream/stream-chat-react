...
```jsx
  ...
  const customGiphyPreview = (message: GiphyPreviewMessageProps) => {
    const streamMessage: StreamMessage = message.message;

    const handleAction = useActionHandler(streamMessage)
  
    if (!streamMessage.attachments) return <div></div>;
  
    return (
      <Attachment
        actionHandler={handleAction}
        AttachmentActions={(props) => <CustomAttachmentActions {...props} />}
        attachments={streamMessage.attachments}
      />
    );
  }
