...
In this example we will demonstrate how to make a custom `TypingIndicator` using the default component as a guide.

## Create the Component

Using the `typing` object provided by the `TypingContext` we can get data about the user typing and use `name` and `role` in this example. The `threadList` prop will allow us to conditionally render the typing indicator in the main message list or at the bottom of a thread.
