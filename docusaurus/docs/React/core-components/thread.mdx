...
rendered UI can be overriden with custom components either drawn from the `ComponentContext` or supplied via props.
...
component. `Window` handles width changes in the main channel to ensure a seemless user experience when opening and
closing a `Thread`.
...
and [`MessageInput`](../message-input-components/message-input.mdx) can be overriden via props if you desire different
