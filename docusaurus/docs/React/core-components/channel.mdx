...
- `TypingContext` - object of currently typing users (i.e., `typing`)
...
However in the absense of a `ChannelList`, the `channel` prop is required. By default, the `ChannelList` sets the active `channel`
...
Any child of the `Channel` component has access to the four contexts refereneced above. Each context can be accessed with
...
The currenlty active `StreamChat` `channel` instance to be loaded into the `Channel` component and referenced by its children.
