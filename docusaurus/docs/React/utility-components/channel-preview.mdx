...
If the component's `channel` is the active (selected) one.
...
The image of the `channel` to display.
...
The title of `channel` to display.
...
The latest message preview text to display.
...
Text truncation limit for the latest message.
