...
The `ChannelStateContext` is a one of the context providers exposed by the `Channel` component and so the value is consumable by all of the children of `Channel`
The context value provides all of the state properties and logic for a `channel`.
The `ChannelStateContext` also exposes the hook `useChannelStateContext`.
