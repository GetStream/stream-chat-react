...
React mutable ref placed on the `MessageActionsBox` container `div`.
...
React mutable ref placed on the `InfiniteScroll` container `ul`.
...
React mutable ref placed on the `Modal` container `div`.
...
React mutable ref placed on the `ReactionSelector` reaction `li`.

| Type                                   |
| -------------------------------------- |
| React.MutableRefObject<HTMLLIElement\> |
...
React mutable ref placed on the `EditMessageForm` `button`.
...
Function to toggle the 'triggerFocus' state on 'ChannelStateContext'.
...
Boolean used to trigger focus of the previous focused message if keyboard navigation is used.
