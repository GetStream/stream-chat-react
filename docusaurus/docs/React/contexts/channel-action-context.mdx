...
The `ChannelActionContext` is a one of the context providers exposed by the `Channel` component and so the value is consumable by all of the children components of `Channel`. The context value provides all of the action properties and handlers for a `channel`.
The `ChannelActionContext` exposes the hook `useChannelActionContext`.
