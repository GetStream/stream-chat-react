...
`ReactionsList` component so the display of processed reactions has the same emoji objects.
:::

```jsx
const customReactions = [
  // add array of custom `MinimalEmoji` type objects here
];
  ...
  return (
    <div>
      ...
      {message.latest_reactions && (
        <ReactionsList reactions={message.latest_reactions} reactionOptions={customReactions} />
      )}
    </div>
  );
};
