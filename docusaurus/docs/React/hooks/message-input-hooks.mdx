...
A [custom hook](https://github.com/GetStream/stream-chat-react/blob/master/src/components/MessageInput/hooks/useMessageInputState.ts) that is a reducer hook and compiles all the state information for the input and returns the current state and a dispatch method.
