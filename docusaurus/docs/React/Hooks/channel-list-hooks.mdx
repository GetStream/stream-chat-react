...
The `ChannelList` displays a list of `channels` via a state variable of the same name. The component provides a variety of hooks that are used within the component to handle events related to updates for a `channel`.
The list's state variable is then updated accordingly.
...
To view the list of these custom handler functions (if there is one) to override, see the `ChannelList` [documentation](../core-components/channel_list#event-listeners).
...
A [custom hook](https://github.com/GetStream/stream-chat-react/blob/master/src/components/ChannelList/hooks/usePaginatedChannels.ts) that handles when a user's status changes (ex: online, offline, etc.).

#### useUserPresenceChangedListener

A [custom hook](https://github.com/GetStream/stream-chat-react/blob/master/src/components/ChannelList/hooks/useUserPresenceChangedListener.ts) that handles pagination and the querying of the channels for the list.
