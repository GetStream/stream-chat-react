...
- [`MessageBounceOptions`](https://github.com/GetStream/stream-chat-react/blob/master/src/components/MessageBounce/MessageBounceOptions.tsx) -
...
Use this prop to easily override the text displayed in the modal dialog for the bounced messages, without fully implementing a custom `MessageBounceOptions` component:

```jsx
import { MessageBounceOptions } from 'stream-react-chat';

function MyCustomMessageBounceOptions(props) {
  return <MessageBounceOptions {...props} text='My custom text' />;
}
```

Then override the default `MessageBounceOptions` component with your custom one:

```jsx
<Channel MessageBounceOptions={MyCustomMessageBounceOptions}>
  <Window>
    <ChannelHeader />
    <MessageList />
    <MessageInput />
...
The Message UI component will pass this callback to close the modal dialog `MessageBounceOptions` are rendered in.
