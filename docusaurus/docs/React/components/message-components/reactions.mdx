...
The SDK comes with built-in support for adding reactions to messages. The component library provides thre default components to enable reaction selection and display:
...
- [`ReactionsList`](https://github.com/GetStream/stream-chat-react/blob/master/src/components/Reactions/ReactionsList.tsx) - displays the reactions added to a message (and [`ReactionsListModal`](https://github.com/GetStream/stream-chat-react/blob/master/src/components/Reactions/ReactionsListModal.tsx))
...
See [Custom Reactions](../../guides/theming/reactions.mdx) guide on how to change default reactions or how to change default behavior of `ReactionsList` and `ReactionSelector` components.
