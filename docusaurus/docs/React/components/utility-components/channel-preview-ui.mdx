...
Marks active channel read before switching the active channel state to the clicked preview's channel. This means that active channel being left is marked read before navigating to another channel. Disabled if markActiveChannelReadOnReenter is enabled.
...
That means:
1. an active channel is be marked unread, left and on return marked read.
