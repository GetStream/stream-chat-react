...
Function that triggers the search for URLs and their enrichment. It is available only if link preview rendering is enabled. Link previews are by default disabled.
