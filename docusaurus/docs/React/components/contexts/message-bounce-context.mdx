...
In most cases when using the default Message UI component implementation you are not going to deal with the `MessageBounceContext` directly. However if you are customizing the Message UI component, or providing a custom `MessageBounceOptions`, the callbacks provided by this context come in handy.
...
Use these callbacks to implement your custom `MessageBounceOptions`. Normally this component displays three options: edit the message before sending it again, send the message again without changes (this can be useful if you are using the "Bounce then flag" moderation flow), and delete the message.

```jsx
import { useMessageBounceContext } from 'stream-chat-react';

function MyCustomMessageBounceOptions({ onClose }) {
  const { message, handleEdit, handleSend, handleDelete } = useMessageBounceContext();
  return (
    <>
