...
sequenceDiagram
  ...
  Client State->>WS Client: attaches listeners
  Channel->>WS Client: attaches listeners
  WS Client->>Client State: receives a message and triggers listeners
  Client State->>Client State: updates state;
  WS Client->>Channel: receives a message and triggers listeners
  Channel->>Client State: reaches for updated client state
  Channel->>Channel: updates state
```
