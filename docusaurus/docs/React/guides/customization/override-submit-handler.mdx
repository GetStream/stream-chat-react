...
In case you would like to store custom data on your message objects, you can do it passing additional parameter to the [`sendMessage`](../../components/contexts/channel-action-context.mdx#sendmessage) function you retrieve from `ChannelActionContext`.

And so now the `overrideSubmitHandler` implementation may change as follows:
...
<img src={MessageCustomPayload} alt={'Custome message payload'}/>
