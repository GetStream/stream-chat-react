...
The React SDK maintains channel read state for own user in separate variable `ownChannelReadState`. Channel read for all the users can be accessed via `channel.state.read` mapping.

### Own user's read state

The read state is maintained by `Channel` component and shared with its children via `ChannelStateContext` as `ownChannelReadState`. The state format is as follows:
...
In the SDK, the `read` and `ownChannelReadState` state can be accessed via [`useChannelStateContext` consumer](../../components/contexts/channel_state_context#read):
...
const Component = (props) => {
    ...
    const { read, channel, ownChannelReadState } = useChannelStateContext();
...
|------------------------|-----------|----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `updateLocalReadState` | `boolean` | yes      | Signal, whether the `ownChannelReadState` should be updated. The local state update is prevented when the Channel component is mounted. This is in order to keep the UI indicating the original unread state, when the user opens a channel. If the value for `updateLocalReadState` is not provided, the state is updated. |
|
