...
const renderWithProvider = (ui) => render(<WithAudioPlayback>{ui}</WithAudioPlayback>);
...
describe('WithAudioPlayback + useAudioPlayer', () => {
  it('useAudioPlayer returns undefined when src is missing', () => {
    let seen;
    renderWithProvider(<RegisterPlayer onReady={(p) => (seen = p)} params={{}} />);
    expect(seen).toBeUndefined();
  });

  it('creates an AudioPlayer when src is provided and wires the underlying audio element', () => {
    let player;
    renderWithProvider(
      <RegisterPlayer
        onReady={(p) => (player = p)}
    ...
    expect(createdAudios[0].src).toBe('https://example.com/a.mp3');
  ...
  it('memoization: same props -> same player instance; changing src -> new instance', () => {
    ...
    const { rerender } = renderWithProvider(
      <RegisterPlayer onReady={(p) => (first = p)} params={props} />,
    );
  ...
  it('unmounting WithAudioPlayback unsubscribes audio element listeners and pauses', () => {
    ...
    // Spy on removeEventListener and pause on this element
    const removeSpy = jest.spyOn(audio, 'removeEventListener');
    const pauseSpy = jest.spyOn(audio, 'pause');

    // Sanity: listeners must have been added
    const addSpy = jest.spyOn(audio, 'addEventListener');
    expect(addSpy).not.toBeNull(); // just to force TS/JS to keep it
