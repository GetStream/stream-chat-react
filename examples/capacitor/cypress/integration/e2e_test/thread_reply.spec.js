...
it('thread reply a message() - type into a DOM element', () => {
  cy.wait(10000)
  cy.xpath("//div[contains(@class,'message--has-text')]").last().invoke('mouseover')
    .click({force:true})
  cy.dataTestContains('message-inner').last().invoke('show')
    .should('exist')
  ...
  // hiding sub-menu Reply
    cy.dataTestContains('thread-action').last().invoke('mouseover').then(element => {
      console.log(element)
      ...
      cy.xpath("//textarea[contains(@placeholder,'Type your message')]").last().then(element => {
      console.log(element)
      const cls = element.attr('class')
      cy.wrap(element).type(input, { delay: 100 }).should('have.class', cls)
        .type('{enter}')
      // close thread reply
